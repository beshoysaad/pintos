...
fsutil_extract (char **argv UNUSED) 
{
  ...
  for (;;)
    {
      ...
      bcache_read (src, sector++, header, BLOCK_SECTOR_SIZE, 0);
          ...
          while (size > 0)
            {
              int chunk_size = (size > BLOCK_SECTOR_SIZE
                                ? BLOCK_SECTOR_SIZE
                                : size);
              bcache_read (src, sector++, data, BLOCK_SECTOR_SIZE, 0);
  ...
  bcache_write (src, 0, header, BLOCK_SECTOR_SIZE, 0);
  bcache_write (src, 1, header, BLOCK_SECTOR_SIZE, 0);
...
fsutil_append (char **argv)
{
  ...
  bcache_write (dst, sector++, buffer, BLOCK_SECTOR_SIZE, 0);
  ...
  bcache_write (dst, sector, buffer, BLOCK_SECTOR_SIZE, 0);
  bcache_write (dst, sector, buffer + 1, BLOCK_SECTOR_SIZE, 0);
