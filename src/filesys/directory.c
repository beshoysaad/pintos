...
dir_is_empty (struct dir *dir)
{
  struct dir_entry e;
  off_t offs = 0;
  while (inode_read_at (dir->inode, &e, sizeof e, offs) == sizeof e)
    {
      offs += sizeof e;
      if (e.in_use)
	{
	    ...
	    enum entry_type type)
{
  ...
  struct dir *cur_dir = dir_open (dir->inode);

  char *name_cpy = malloc (strlen (name) + 1);
  memcpy (name_cpy, name, strlen (name) + 1);

  for (token = strtok_r (name_cpy, "/", &save_ptr); token != NULL; token =
	 ...
	 enum entry_type type)
{
  ...
  // Make sure this was last in name
  ...
  e.type = type;
  success = inode_write_at (cur_dir->inode, &e, sizeof e, ofs) == sizeof e;
...
dir_remove (struct dir *dir, const char *name)
{
  ...
  e.in_use = false;
  if (inode_write_at (cur_dir->inode, &e, sizeof e, ofs) != sizeof e)
    goto done;
